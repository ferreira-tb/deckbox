...
pub struct Db_CardAttribute(ygo::CardAttribute);

impl FromSql<Text, Sqlite> for Db_CardAttribute {
  fn from_sql(bytes: <Sqlite as Backend>::RawValue<'_>) -> de::Result<Self> {
    let value = <String as FromSql<Text, Sqlite>>::from_sql(bytes)?;
    Ok(Db_CardAttribute(ygo::CardAttribute::from_str(
      value.as_str(),
    )?))
  }
}

impl ToSql<Text, Sqlite> for Db_CardAttribute
where
