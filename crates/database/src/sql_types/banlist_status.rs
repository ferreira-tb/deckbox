...
pub struct Db_BanlistStatus(ygo::BanlistStatus);

impl FromSql<Text, Sqlite> for Db_BanlistStatus {
  fn from_sql(bytes: <Sqlite as Backend>::RawValue<'_>) -> de::Result<Self> {
    let value = <String as FromSql<Text, Sqlite>>::from_sql(bytes)?;
    Ok(Db_BanlistStatus(ygo::BanlistStatus::from_str(
      value.as_str(),
    )?))
  }
}

impl ToSql<Text, Sqlite> for Db_BanlistStatus
where
