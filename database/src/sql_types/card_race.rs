...
pub struct Db_CardRace(ygo::CardRace);

impl FromSql<Text, Sqlite> for Db_CardRace {
  fn from_sql(bytes: <Sqlite as Backend>::RawValue<'_>) -> de::Result<Self> {
    let value = <String as FromSql<Text, Sqlite>>::from_sql(bytes)?;
    Ok(Db_CardRace(ygo::CardRace::from_str(value.as_str())?))
  }
}

impl ToSql<Text, Sqlite> for Db_CardRace
where
